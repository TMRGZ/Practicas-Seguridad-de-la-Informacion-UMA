...
# A resolver por el alumno
pk = PKCS1_OAEP.new(private)
desc = pk.decrypt(cipherRSA)
h = SHA256.new(desc)  # Cambiar por texto descifrado
...
cipherA = DES.new(K_AT, DES.MODE_ECB)
cifrado_K_AT = cipherA.encrypt(pad(M, BLOCK_SIZE_8))
...
decipherT = DES.new(K_AT, DES.MODE_ECB)
M = unpad(decipherT.decrypt(msg_K_AT), BLOCK_SIZE_8)
cipherB = DES.new(K_BT, DES.MODE_ECB)
cifrado_K_BT = cipherB.encrypt(pad(M, BLOCK_SIZE_8))
...
decipherB = DES.new(K_BT, DES.MODE_ECB)
M = unpad(decipherB.decrypt(msg_K_BT), BLOCK_SIZE_8)
...
# B ######### A realizar por el alumno ##
desc = PKCS1_OAEP.new(private)
Texto_M = desc.decrypt(msg_Cipher_M)
h2 = HMAC.new(K_AB, digestmod=SHA256)
h2.update(Texto_M)
try:
    h2.verify(msg_HMAC_M)
    print(Texto_M.decode("utf-8", "ignore"))
except ValueError:
    print("Mensaje malo")
